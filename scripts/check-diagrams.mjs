...
// Graphviz is required to render PNGs from .dot sources. Local machines without
// `dot` skip with a clear message so this never blocks a commit.
...
const tempDir = mkdtempSync(join(tmpdir(), 'connect-four-diagrams-'));
...
// which would produce stale-PNG false positives. The .dot sources are the
// source of truth; PNGs are for in-browser viewing.
try {
